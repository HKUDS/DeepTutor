...
def _decode_text(raw: bytes) -> str:
    ...
    async def export_offline_package(self, document_id: str) -> "Path":
        ...
        - translated segments for this book (from TranslationMemory)
        - an ECDICT subset containing words explicitly found in this book
...
class ImmersiveReadingService(TranslationMixin):
    ...
    async def lookup_dictionary(self, word: str, context: str = "") -> dict[str, Any]:
        ...
        Keep dictionary lookup strictly local by delegating to ``lookup_word``,
        which already follows the local-first policy (ECDICT + local Ollama).
        """
        result = await self.lookup_word(word, context)
        return result.model_dump(mode="json")
    ...
    def _resolve_ollama_model(
        ...
        Exact configured preference always wins if installed (e.g. qwen3.5:4b).
        When *for_translation* is True and a translation model is requested,
        resolves to available Hy-MT2 / hunyuan-mt variants.
    ...
    async def _enrich_with_chinese(self, result: DictionaryResult) -> DictionaryResult:
        ...
        ECDICT provides word-level Chinese translations; when they are already
        present this helper is skipped. When only English-only definitions are
        available, this asks the local Ollama model to
    ...
    async def lookup_word(self, word: str, context: str = "") -> DictionaryResult:
        ...
        data locally. The local model is used only when ECDICT has no usable
        entry.
        """
    ...
    def update_mn4_writeback_status(self, ids: list[str], status: str) -> int:
        ...
        now = time.time()
        for w in writebacks:
            if w.id in ids and w.status != status:
                w.status = status
                w.updated_at = now
                updated_count += 1
        if updated_count > 0:

    ...
    with zipfile.ZipFile(epub_path) as archive:
        ...
        for sid in spine_ids:
            if sid not in manifest:
                continue
            manifest_href, media_type = manifest[sid]
            if "xhtml" not in media_type and not manifest_href.endswith(
                (".html", ".xhtml", ".htm")
            ):
                continue
            full_path = (opf_dir / manifest_href).as_posix()
            try:
            ...
            if not chap_title:
                chap_title = Path(manifest_href).stem

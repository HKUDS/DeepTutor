...
class LibraryEntry(BaseModel):
    ...
    reviewer_note: str = ""
    source_scope: Literal["personal", "kids_upload"] = "personal"
    created_at: float = Field(default_factory=time.time)
    updated_at: float = Field(default_factory=time.time)


class LibraryIndex(BaseModel):
    """Top-level multi-scope index for all documents in the workspace."""
...
class KidsDevicePairing(BaseModel):
    """Short-lived one-time code generated by parent to pair a child device."""

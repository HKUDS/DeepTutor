...
def _issue_session(
    manager: KidsManager, profile_id: str, response: Response, device_name: str = "Kids Device"
) -> dict[str, Any]:
    session, token = manager.create_device_session(
        profile_id, ttl_seconds=_SESSION_TTL_SECONDS, device_name=device_name
    )
    response.set_cookie(
        _SESSION_COOKIE,
        token,
    ...
    if "kids_family" not in entry.scopes or entry.kids_review_status != "approved":
        raise HTTPException(
            ...
            detail={
                ...
                "message": "This book is not available in the family kids library",
            },
        )
    return assignment
...
async def bootstrap(
    ...
    """Expose session status without leaking full profile list to unauthenticated clients."""
    manager = get_kids_manager()
    token = _extract_token(authorization, dt_kids)
    if token:
        session = manager.validate_device_session(token)
        if session:
...
@router.post("/pair")
async def pair_device(request: PairRequest, response: Response) -> dict:
    """Redeem a 6-digit one-time pairing code generated by the parent."""
    manager = get_kids_manager()
    try:
        session, token, profile = manager.redeem_pairing_code(
            request.code, device_name=request.device_name
        )
    except ValueError as exc:
...
async def get_profile_public_info(profile_id: str) -> dict:
    """Get basic info for a single profile when accessed directly via /kids/p/{profileId}."""
    manager = get_kids_manager()
...
async def get_kids_book(
    ...
    allowed_ids = {section.id for section in allowed_sections}
    if allowed_sections and progress.current_section_id not in allowed_ids:
        progress.current_section_id = allowed_sections[0].id
        progress.current_section_index = allowed_sections[0].index
        progress.epub_cfi = ""
        progress.section_href = ""

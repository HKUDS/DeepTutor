...
A connected **MarginNote 4** library is filled by its add-on: connect the library in the Knowledge Center, pair a device from the library's **Devices** tab, and paste the one-time credential into the add-on together with your server URL and the library name. The add-on sources and build script live in [`packaging/marginnote4-addon/`](packaging/marginnote4-addon/) — run `build.sh` there to produce the `.mnaddon` package, then open it in MarginNote 4. The package is unsigned, so enable "allow unauthenticated add-ons" in MarginNote 4 settings first.

...
test.describe('sidebar sessions', () => {
  ...
  test('置顶 / 重命名 / 删除真正改侧栏', async ({ page }) => {
    ...
    const greeting = page.locator('.taskItem').filter({ hasText: 'Greeting' })
    await greeting.hover()
    await greeting.getByRole('button', { name: '置顶' }).click()
    await expect(page.locator('.taskItem').first()).toHaveClass(/taskItemPinned/)
    await expect(page.locator('.taskItem').first().locator('.taskText')).toHaveText('Greeting')

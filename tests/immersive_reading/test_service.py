...
async def test_dictionary_lookup_falls_back_to_local_model_when_ecdict_misses(
    ...
    """Words absent from ECDICT fall back to local LLM."""

    def no_local(_word: str):
        return None

    monkeypatch.setattr(reading_service, "_local_dictionary_lookup", no_local)

    import aiohttp

    async def _ensure(for_translation: bool = False) -> str:
        return "qwen3.5:4b"
    ...
    monkeypatch.setattr(aiohttp, "ClientSession", lambda **kw: _FakeSession())
    monkeypatch.setattr(reading_service, "_ensure_ollama_ready", _ensure)
...
async def test_lookup_dictionary_delegates_to_lookup_word_for_strict_local_policy(
    ...
    """Ensure lookup_dictionary invokes lookup_word, preserving the strict-local
    policy (ECDICT first, local model fallback)."""
    called_with = []
...
@pytest.mark.asyncio
async def test_translate_rejects_non_ollama_providers_in_strict_local_mode(
    reading_service: ImmersiveReadingService, monkeypatch
) -> None:
    """Non-Ollama providers are blocked in strict-local mode."""
    from types import SimpleNamespace
    ...
    called: dict[str, int] = {"complete": 0}
    ...
    with pytest.raises(LLMConfigError):
        await reading_service.translate("cloud", "Russian")
...
async def test_translate_retries_once_and_never_returns_broken_protection(
    ...
    monkeypatch.setattr(reading_service, "_ensure_ollama_ready", _ready)

...
def test_normalize_epub_href_collapses_relative_segments() -> None:
    ...
    assert (
        normalize_epub_href("../ch1.xhtml", base_dir="OEBPS/Text", opf_dir="OEBPS") == "ch1.xhtml"
    )
...
def test_apply_source_hrefs_shares_href_across_split_sections() -> None:
    sections = [
        ...
        ReadingSection(
            id="section_0004", title="Chapter 2: The Harbor", index=3, source_start=4, source_end=6
        ),
    ]
...
def test_apply_source_hrefs_does_not_clobber_existing_values() -> None:
    sections = [
        ReadingSection(
            id="section_0001", title="Chapter 1: The Observatory", index=0, source_href="kept.xhtml"
        ),
    ]
    changed = apply_source_hrefs(sections, build_epub(), reading_mode="chapters")
...
def test_resolve_section_for_href_prefers_current_then_first_leaf() -> None:
    ...
    assert (
        resolve_section_for_href(
            sections, "OEBPS/chapter-1.xhtml", preferred_section_id="leaf-b"
        ).id
        == "leaf-b"
    )
...
def test_resolve_section_titles_replaces_filename_stems() -> None:
    ...
    sections = [
        ReadingSection(
            id="section_0001", title="chapter-1", index=0, source_href="chapter-1.xhtml"
        ),
        ReadingSection(
            id="section_0002", title="chapter-2", index=1, source_href="chapter-2.xhtml"
        ),
        ReadingSection(
            id="section_0003", title="chapter-3", index=2, source_href="chapter-3.xhtml"
        ),
    ]
...
def test_section_needs_title_detects_stem() -> None:
    assert section_needs_title(
        ReadingSection(
            id="s1", title="index_split_004", index=0, source_href="index_split_004.xhtml"
        )
    )

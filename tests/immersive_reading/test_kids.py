...
def protected_profile(document_with_cover, kids_manager):
    ...
    kids_manager.assign_book(profile.id, document_with_cover["id"], available_through_section_index=2)
    return profile
...
def test_child_resources_require_a_persisted_device_session(
    ...
    assert client.get("/api/v1/kids/library", headers={"X-Profile-Id": protected_profile.id}).status_code == 401
    ...
    assert client.get(f"/api/v1/kids/books/{document_id}/cover", headers=auth_headers(token)).status_code == 200

    assert client.post("/api/v1/kids/session/logout", headers=auth_headers(token)).status_code == 200
...
def test_children_receive_an_epub_limited_to_assigned_sections(
    ...
    with zipfile.ZipFile(io.BytesIO(response.content)) as archive:
        ...
        combined = "\n".join(archive.read(name).decode("utf-8") for name in names if name.endswith(".xhtml"))
...
def test_daily_limit_uses_capped_server_elapsed_time(
    kids_manager, protected_profile
) -> None:

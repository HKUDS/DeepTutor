...
def test_store_uses_versioned_files_and_leaves_no_temporary_files(tmp_path: Path) -> None:
    ...
    store.save_state({"operation_state": "idle"})
...
def test_state_and_catalog_cache_round_trip(tmp_path: Path) -> None:
    store = CodexCredentialStore(tmp_path)

    store.save_state({"operation_state": "pending", "operation_id": "op-1"})
    store.save_catalog_cache({"etag": '"v1"', "models": [{"slug": "gpt-5.6-sol"}]})

    assert store.load_state()["operation_state"] == "pending"
    assert store.load_state()["generation"] == 0

...
- **بيئة تشغيل واحدة لجميع الأوضاع** — تشترك Chat وAsk Questions وQuiz وResearch وVisualize وSolve وMastery Path وImmersive Reading في بيئة تشغيل واحدة للقدرات وسياق الجلسة، مع احتفاظ كل منها بحلقات وخطوط أنابيب مصممة لغرضها.
...
- **الوكلاء الفرعيون والشركاء** — استشارة واجهة CLI برمجية حية (Claude Code أو Codex أو Gemini أو Antigravity أو Kimi أو opencode أو MiMo) أو Partner مباشرةً من أي دور (أو استيراد محادثاتهم السابقة)، وتشغيل رفاق IM دائمين على نفس الدماغ.
- **معرفة متعددة المحركات** — مكتبات RAG مُصدَّرة عبر LlamaIndex وPageIndex وGraphRAG وLightRAG، أو LightRAG Server عن بُعد، أو مكتبة Tencent IMA أو MarginNote 4، أو مخزن Obsidian مرتبط، مع تحليل مستندات قابل للتوصيل.
...
deeptutor init     # يطلب المنافذ + مزود LLM + تضمين/بحث اختياري
...
Chat هي أيضاً نقطة انطلاق للقدرات الأعمق: **Ask Questions** لبطاقة استيضاح مدركة للسياق، و**Quiz** لتوليد الأسئلة، و**Visualize** للمخططات / الرسوم البيانية / الرسوم المتحركة، و**Mastery Path** لتدفقات خطط التعلم، و**Immersive Reading** لقراءة ملفات PDF وEPUB وغيرها من المستندات بجانب المحادثة، مع إسناد إلى الصفحة/الفصل، وحفظ دائم لموضع القراءة والمخططات الهيكلية، وتعليقات توضيحية، وإجراءات على النص المحدد. أما **Research** للتقارير المستشهَد بها و**Solve** للتفكير المعمق فتحت *المزيد من القدرات*.
...
فضاء التعلم هو طبقة المكتبة والتنظيم والتخصيص. تجمع **دوراتي** محادثات كل مادة وتُبقي محادثات المدرّس متداخلة تحت محادثتها الأصلية؛ ويتيح سجل المحادثات التصفية حسب الدورة أو نوع المحادثة، وتثبيت الجلسات أو أرشفتها أو نقلها. وتضم **المحادثات والمواد** أيضاً دفاتر الملاحظات — بسجلات يمكن نقلها أو نسخها بين الدفاتر وتصديرها إلى Markdown — وبنك أسئلة يحتفظ بإجابتك والإجابة المرجعية والشرح. أما **التخصيص** فيضم مسارات الإتقان والشخصيات والمهارات (أدلة `SKILL.md`) و**خدمات MCP** التي تُثبَّت بنقرة واحدة و**تطبيقات CLI** من كتالوج [CLI-Anything](https://github.com/HKUDS/CLI-Anything)، ولكل تطبيق دليل استخدام يُحمَّل عند الطلب. كل شيء هنا يمكن إعادة استخدامه من Chat وPartners وCo-Writer وBook.
...
**أول مستخدم مسجَّل يصبح مشرفاً** ويمتلك كتالوجات النماذج وبيانات اعتماد المزود وقواعد المعرفة المشتركة والمهارات والكتب المشتركة المرجعية والمنح لكل مستخدم. يحصل الجميع الآخرون على مساحة عمل معزولة وصفحة إعدادات منقوصة — تظهر النماذج وقواعد المعرفة والمهارات المعيّنة كخيارات محدودة النطاق وللقراءة فقط، وليس كمفاتيح API خام. تُعيَّن صلاحية إنشاء الكتب وصلاحيات القراءة أو التحرير التعاوني الافتراضية/الخاصة بكل كتاب بشكل منفصل ضمن **الوصول إلى الكتب**؛ ويظل حذف الكتب المشتركة مقصوراً على المشرف.
...
| `deeptutor run <capability> <message>` | تشغيل دور قدرة واحدة (`chat` و`ask_questions` و`deep_solve` و`deep_question` و`deep_research` و`visualize` و`math_animator` و`mastery_path` و`immersive_reading`)؛ أضف `--format json` لإخراج NDJSON |
...
في عمليات النشر متعددة المستخدمين، تصل عمليات الاستيراد إلى مكتبة المهارات الخاصة بالمستدعي؛ وتبقى المهارات التي يعيّنها المشرف مقيّدة بالمنح وللقراءة فقط.

...
- **Единая среда выполнения для всех режимов** — Chat, Ask Questions, Quiz, Research, Visualize, Solve, Mastery Path и Immersive Reading работают в единой среде выполнения и используют общий контекст сессии, но сохраняют специализированные циклы и конвейеры для своих задач.
...
- **Субагенты и Partners** — консультируйте живой инструмент кодирования CLI (Claude Code, Codex, Gemini, Antigravity, Kimi, opencode или MiMo) или Partner из любого хода (или импортируйте их прошлые разговоры) и запускайте постоянных IM-компаньонов на том же мозге.
...
Чат также является точкой запуска более глубоких возможностей: **Ask Questions** для контекстной карточки с уточняющим вопросом, **Quiz** для генерации вопросов, **Visualize** для графиков / диаграмм / анимаций, **Mastery Path** для учебных планов и **Immersive Reading** для чтения PDF, EPUB и других документов рядом с потоком чата — с привязкой к странице/главе, сохранением позиции и оглавления, аннотациями и действиями над выделенным текстом. **Research** для цитируемых отчётов и **Solve** для обоснованных рассуждений находятся в разделе *Дополнительные возможности*.
...
Мои Агенты превращают других агентов в контекст для DeepTutor и делают две разные вещи. **Подключите живого агента** — Claude Code, Codex, Gemini, Antigravity, Kimi, opencode или MiMo Code CLI на вашей машине, или одного из ваших Партнёров — и обращайтесь к нему изнутри хода чата: DeepTutor фактически *запускает* другого агента и транслирует его работу в панель Activity через инструмент `consult_subagent`. Выберите его чипом агента (или введите `@`) и задайте, сколько раундов может занять обращение.
...
Пространство Обучения — это уровень библиотеки, организации и персонализации. **My courses** группирует беседы по предметам и размещает ветки репетитора внутри родительской ветки; в Chat History можно фильтровать по курсу или типу ветки, а также закреплять, архивировать и перемещать сессии. **Разговоры и материалы** также содержат блокноты — записи можно перемещать или копировать между блокнотами и экспортировать в Markdown — и банк вопросов, который хранит ваш ответ, эталонный ответ и объяснение. **Персонализация** содержит пути мастерства, персонажей, навыки (сценарии `SKILL.md`), устанавливаемые одним кликом **MCP-сервисы** и **CLI-приложения** из каталога [CLI-Anything](https://github.com/HKUDS/CLI-Anything), причём руководство по использованию каждого приложения загружается по требованию. Всё здесь можно повторно использовать из Chat, Partners, Co-Writer и Book.
...
**Первый зарегистрированный пользователь становится администратором** и владеет каталогами моделей, учётными данными провайдеров, общими базами знаний, навыками, каноническими общими книгами и грантами для пользователей. Все остальные получают изолированное рабочее пространство и отредактированную страницу Настроек — назначенные модели, KB и навыки отображаются как ограниченные опции только для чтения, а не как сырые API-ключи. Право создавать книги, а также доступ по умолчанию и для отдельных книг в режиме чтения или совместного редактирования назначаются отдельно в разделе **Book access**; удалять общие книги по-прежнему может только администратор.
...
| `deeptutor run <capability> <message>` | Запустить один ход возможности (`chat`, `ask_questions`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`, `immersive_reading`); добавьте `--format json` для вывода NDJSON |
...
В многопользовательских развёртываниях импортированные навыки попадают в личную библиотеку пользователя, выполнившего импорт; назначенные администратором навыки остаются ограниченными областью гранта и доступны только для чтения.

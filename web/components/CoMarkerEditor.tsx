...
export default function CoWriterEditor({ initialValue = '' }: CoWriterEditorProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Only show errors in development mode
        if (process.env.NODE_ENV === 'development') {
          console.debug(`${t('Failed to initialize')} (backend may not be running):`, error.message)
        }
      }
    }
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      alert('PDF export failed, please try again. Check console for error details.')
    } finally {
      setIsProcessing(false)
    }
  }
      ...
      <div className="flex-1 flex flex-col gap-4">
          ...
          <div className="p-2 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] text-slate-400">Live Preview · Synced Scroll</span>
              </div>
            </div>
        ...
        <div className="shrink-0 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden transition-all">
          ...
          >
            ...
            {!isPodcastExpanded && (
              <span className="text-[10px] text-slate-400 px-2">Click to expand</span>
            )}
          </div>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setNarrationError(
                            e?.message || 'Failed to generate narration, please try again.'
                          )
                        } finally {

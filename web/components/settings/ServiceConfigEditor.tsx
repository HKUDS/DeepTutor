...
export function ServiceConfigEditor({ service }: { service: ServiceName }) {
  ...
  const isManagedCodex =
    activeProfile?.managed_by === "openai_codex_oauth";
  const isCodexOAuth =
    service === "llm" &&
    activeProviderValue === "openai_codex" &&
    (activeProviderOption?.auth_mode === "oauth" || isManagedCodex);
            ...
            <div className="space-y-0.5">
              {draft.services[service].profiles.map((profile) => {
                ...
                const isManagedProfile =
                  profile.managed_by === "openai_codex_oauth";
                const isEditing =
...
function ProfileFields({
  ...
  const isCodexOAuth =
    ...
    (providerOption?.auth_mode === "oauth" || isManagedCodex);

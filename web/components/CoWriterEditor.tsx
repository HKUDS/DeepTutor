...
export default function CoWriterEditor({ initialValue = '' }: CoWriterEditorProps) {
  ...
  const [narrationStyle] = useState<'friendly' | 'academic' | 'concise'>('friendly')
  ...
  const [, setTtsVoices] = useState<Array<{ id: string; name: string; description?: string }>>([])
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (isConnected) {
          ...
          try {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.debug('Failed to fetch KBs (backend may not be running):', err.message)
            }
          }
        ...
        // Only show errors in development mode
        if (process.env.NODE_ENV === 'development') {
          console.debug('Failed to initialize (backend may not be running):', error.message)
        }
      }
    }
  ...
  const fetchHistory = () => {
    fetch(apiUrl('/api/v1/co_writer/history'))
      ...
      .catch(err => {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.debug('Failed to fetch history (backend may not be running):', err.message)
        }
        setBackendConnected(false)
      })
  }
    ...
    try {
      ...
      if (action === 'automark') {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `${t('Failed to auto-mark text')}: ${res.status} ${res.statusText}\n${errorText}`
          )
        }

        const data = await res.json()
        if (!data.marked_text) {
          throw new Error(t('Invalid response: missing marked_text field'))
        }
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `${t('Failed to edit text')}: ${res.status} ${res.statusText}\n${errorText}`
          )
        }

        const data = await res.json()
        if (!data.edited_text) {
          throw new Error(t('Invalid response: missing edited_text field'))
        }
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      alert(t('PDF export failed, please try again. Check console for error details.'))
    } finally {
      setIsProcessing(false)
    }
  }
  ...
  const Divider = () => <div className="w-px h-5 bg-slate-200 dark:bg-slate-600 mx-1" />
        ...
        >
          ...
          <div className="p-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 flex justify-between items-center">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {hideAiMarks ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
              </button>
              <Divider />
              <button
            ...
            >
              <ReactMarkdown
                ...
                components={{
                  ...
                  u: ({ ...props }) => (
                    <u className="underline decoration-2 decoration-slate-400" {...props} />
                  ),
                  span: ({ ...props }) => {
                  ...
                  p: ({ children, ...props }) => {
                    ...
                    const hasBracket = childArray.some((child: any) => {
                      if (React.isValidElement(child)) {
                        ...
                        return childProps?.['data-rough-notation'] === 'bracket'
                      }
                      return false
                    })
                ...
                <div className="flex-1 space-y-2">
                  <div className="flex gap-2">
                    <button
                      onClick={async () => {
                        if (!content.trim()) {
                          setNarrationError(t('Current note is empty, cannot generate narration.'))
                          return
                        }
                        ...
                        try {
                          ...
                          setNarrationError(
                            e?.message || t('Failed to generate narration, please try again.')
                          )
                        } finally {
                  ...
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg p-2 h-32 overflow-y-auto text-xs text-slate-700 dark:text-slate-300">
                    {narrationScript ? (
                      <p className="whitespace-pre-wrap leading-relaxed">{narrationScript}</p>
                    ) : (

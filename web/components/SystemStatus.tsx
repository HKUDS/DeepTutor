...
export default function SystemStatus() {
  ...
  const getStatusIcon = (status: string, isBackend: boolean = false) => {
    if (isBackend) {
      return backendConnected ? <Wifi className="w-3 h-3" /> : <WifiOff className="w-3 h-3" />
    }
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs ${getStatusColor(statusData.llm.status)}`}>
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-slate-700 dark:text-slate-200">向量模型</span>
              </div>
              <div className="flex items-center gap-2">
                {statusData.embeddings.status === 'configured' && testResults.embeddings && (
                  <span
                ...
                <span className={`text-xs ${getStatusColor(statusData.embeddings.status)}`}>
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-slate-700 dark:text-slate-200">TTS 模型</span>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs ${getStatusColor(statusData.tts.status)}`}>
                  {getStatusText(statusData.tts.status)}
                </span>
              </div>
            </div>
            {statusData.tts.model && (
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate mb-0.5">
                {statusData.tts.model}
              </div>
            )}
            <div className="text-[10px] text-slate-400 dark:text-slate-500 truncate mb-1.5">
              {statusData.tts.base_url || 'No endpoint'}
            </div>

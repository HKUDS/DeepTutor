...
import { X, BookOpen, ChevronRight, ChevronDown, Loader2, Check, Calendar } from 'lucide-react'
...
export default function NotebookImportModal({
  ...
  const [notebookRecords, setNotebookRecords] = useState<Map<string, NotebookRecord[]>>(new Map())
  const [loadingRecords, setLoadingRecords] = useState<Set<string>>(new Set())
  const [selectedRecords, setSelectedRecords] = useState<Set<string>>(new Set()) // Record IDs
  const [selectedRecordsData, setSelectedRecordsData] = useState<NotebookRecord[]>([])

  const displayTitle = title === 'Import from Notebooks' ? t('Import from Notebooks') : title
  ...
  const fetchNotebooks = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const validNotebooks = (data.notebooks || []).filter((nb: Notebook) => nb.record_count > 0)
  ...
  const fetchRecords = async (notebookId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setNotebookRecords(prev => new Map(prev).set(notebookId, data.records || []))
      }
    } catch (error) {
      console.error(`Failed to fetch records for notebook ${notebookId}:`, error)
    } finally {
  ...
  const toggleRecordSelection = (record: NotebookRecord) => {
    setSelectedRecords(prev => {
      ...
      if (next.has(record.id)) {
        ...
        setSelectedRecordsData(current => current.filter(r => r.id !== record.id))
      } else {
        next.add(record.id)
            ...
            {expandedNotebooks.size === 0 ? (
              ...
              <div className="space-y-4">
                {Array.from(expandedNotebooks).map(nbId => {
                  ...
                  return (
                    <div key={nbId} className="animate-in fade-in slide-in-from-bottom-2">

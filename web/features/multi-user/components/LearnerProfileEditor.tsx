...
export function LearnerProfileEditor({ username }: { username: string }) {
  ...
  async function save() {
    ...
    setMessage("");
    try {
      await setLearnerProfile(username, profile);
      setMessage(t("Saved"));
    } catch (error) {
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {fields.map((field) => (
          <label key={field} className="text-xs text-[var(--muted-foreground)]">
            {t(field === "grade_level" ? "Grade level" : field === "explanation_style" ? "Explanation style" : field.replaceAll("_", " "))}
            <input
              value={profile[field] ?? ""}
              onChange={(event) =>
                setProfile((current) => ({
                  ...current,
                  [field]: field === "age" ? (event.target.value ? Number(event.target.value) : undefined) : event.target.value,
                }))
              }

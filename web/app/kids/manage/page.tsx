...
export default function KidsManagePage() {
  ...
  const handleAssignInteractive = async (bookId: string) => {
    if (!selected) return;
    await kidsAdminApi.assignInteractiveBook(selected.id, { book_id: bookId });
    loadLibrary(selected.id);
  };

  const handleUnassignInteractive = async (bookId: string) => {
    if (!selected) return;
    await kidsAdminApi.unassignInteractiveBook(selected.id, bookId);
    loadLibrary(selected.id);
  };
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        {selected && (
          <div>
            ...
            <div style={{ ...cardStyle, marginBottom: 16 }}>
              ...
              {library.length === 0 ? (
                ...
                library.map((item) => {
                  ...
                  const title = (isInteractive ? item.book?.title : (item.document as Record<string, any>)?.title) || item.assignment.document_title || "Unknown";
                  const targetId = item.assignment.book_id || item.assignment.document_id;
                  return (
                    <div key={item.assignment.id || targetId} style={rowStyle}>
                      <span style={{
                        fontSize: 11,
                        fontWeight: 700,
                        padding: "2px 8px",
                      ...
                      <button
                        onClick={() => {
                          if (!targetId) return;
                          if (isInteractive) {
                            handleUnassignInteractive(targetId);
                          } else {
                            handleUnassign(targetId);
                          }
                        }}
            ...
            {/* Available Interactive Math Books from BookEngine */}
            {availableBooks.length > 0 && (
              <div style={{ ...cardStyle, marginBottom: 16, background: "#faf5ff", border: "1px solid #e9d8fd" }}>
                ...
                {availableBooks
                  .filter((b) => !library.some((l) => l.assignment.book_id === b.id || l.assignment.document_id === b.id))
                  .map((b) => (
                    <div key={b.id} style={rowStyle}>
                      <span style={{ flex: 1, fontSize: 15, fontWeight: 600 }}>{b.title}</span>
                      <span style={{ fontSize: 12, color: "#718096" }}>{b.page_count} pages • {b.status}</span>
                      <button
                        onClick={() => handleAssignInteractive(b.id)}
                        style={{ ...miniBtn, background: "#d6bcfa", color: "#44337a" }}
                      >

...
import SimComposer from "@/components/sim/SimComposer";
import SimMessageList, { type SimMessage } from "@/components/sim/SimMessageList";
import {
  UnifiedWSClient,
...
function looksLikeSimEnded(text: string, stage?: string): boolean {
  if (stage === "debrief") return true;
  const t = (text || "").toLowerCase();
  return t.includes("simulation debrief") || t.includes("房间已结束");
}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 text-sm font-medium text-[var(--foreground)]">
            <GraduationCap className="h-4 w-4 text-[var(--primary)]" aria-hidden />
            Sim
          </div>
          <p className="mt-0.5 text-[11px] text-[var(--muted-foreground)]">
            Trainee as counselor · simulated visitor + supervisor notes
            {dtSessionId ? (
      ...
      {closed && (
        ...
        >
          This sim has ended. Sending is disabled. Start a New sim to continue.
        </div>
      )}

      <div ref={scrollerRef} className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        <div className="mx-auto w-full max-w-3xl">
          <SimMessageList messages={messages} />
        </div>
      </div>
      ...
      <SimComposer
        ...
        inputDisabled={closed}
        endDisabled={sendBlocked || !dtSessionId}
        onDraftChange={setDraft}
        onSend={() => startTurn(draft)}
        onEnd={() => startTurn("结束")}
      />
    </div>
  );

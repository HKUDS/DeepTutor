...
for (const mobile of [false, true]) {
  test(`Watching workspace restores its session and handles provider failures (${mobile ? 'mobile' : 'desktop'})`, async ({
    ...
    await page.route('**/api/**', async route => {
      ...
      const materialId = secondMaterial ? SECOND_MATERIAL_ID : MATERIAL_ID
      if (path === `/api/video-learning/materials/${materialId}`) {
        if (provider === 'invidious' && invidiousOffline) {
          return json({ detail: 'Invidious is offline' }, 400)
        }
        return json(material(provider))
      }
      if (path === `/api/video-learning/materials/${materialId}/transcript/refresh`) {
      ...
      if (path.endsWith('/notes') && request.method() === 'POST') {
        ...
        const note = {
          ...
          material_id: secondMaterial ? SECOND_MATERIAL_ID : MATERIAL_ID,
          body: body.body,

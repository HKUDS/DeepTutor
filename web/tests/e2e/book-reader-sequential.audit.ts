...
import { SCROLL_EDGE_TOLERANCE_PX } from '../../lib/book-reader-navigation'
...
test('book arrows read the current chapter before turning chapters', async ({ page }) => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (pathname === '/api/books/sequential-fixture') {
      return route.fulfill(json({ book, spine: null, pages: summaries, progress }))
    }
  ...
  await expect(page).toHaveURL(/\/books\/sequential-fixture\/pages\/page-2$/)

  await page.getByRole('button', { name: 'Collapse chapters' }).click()
  await expect(page.getByRole('heading', { name: 'Current long chapter' })).toBeVisible()
  ...
  await expect
    ...
    .toBe(100)
  await page.keyboard.press('ArrowRight')
  await expect(page.getByRole('heading', { name: 'Next chapter' })).toBeVisible()
  ...
  await expect
    ...
    .toBeGreaterThan(firstScreen.clientHeight)
  await expect
    .poll(() =>
      reader.evaluate(element => element.scrollHeight - element.clientHeight - element.scrollTop)
    )
    .toBeLessThanOrEqual(SCROLL_EDGE_TOLERANCE_PX)

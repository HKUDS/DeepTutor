...
import { isNavEntryAllowedForLearningPolicy, NAV_BY_HREF } from '../components/sidebar/nav-entries'
...
test('learning policy drives the sidebar and redacted settings document', () => {
  for (const href of ['/chat', '/reading', '/settings']) {
    assert.equal(isNavEntryAllowedForLearningPolicy(NAV_BY_HREF.get(href)!, policy), true, href)
  }
  for (const href of [
    '/partners',
    '/agents',
    ...
    assert.equal(isNavEntryAllowedForLearningPolicy(NAV_BY_HREF.get(href)!, policy), false, href)
  }
  assert.equal(
  ...
  assert.deepEqual(visibleKeys, ['appearance', 'learner-profile', 'about'])
})

...
- **Safe Retirement**: Only retire worktrees whose PRs are merged/closed and whose local state is cleanly archived. Never use `git reset --hard` or `git clean` on dirty unarchived state.
